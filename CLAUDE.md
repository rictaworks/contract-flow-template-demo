...
**現状（2026-09-15時点）：Issue #1（実装）・#3（vite/vitest脆弱性修正）・#5（wrangler v4アップグレード）・#7（P04永久不通過バグ修正）・#8（中止案件のP18クローズ不可バグ修正）・#9（持越し課題の重複登録バグ修正）・#13（デモ共通UI必須4要素+GA4タグの実装）を実装・マージ済み、タグ `v01.01.02`。本番デプロイ済み・本番ユーザーテスト実施済み（Claude Desktopのブラウザツールで実施。詳細は本ファイル「本番デプロイ」節）。#7・#8・#13修正後の再デプロイでの実機再確認も完了済み。既知の未修正バグは無し（2026-09-15時点）。**

**注意（デモ版共通UI必須要素）：新規デモリポジトリでは、`requirements.md`が対象外としていても`20_開発/.claude/agents/demo-common-ui.md`が定める全デモ共通の必須4要素（アンバーバナー・「← デモ一覧へ」戻るリンク・「ご相談はこちら」固定ボタン・`/legal`ページ）とGA4タグ（`G-C04W1XKS16`）は必須。本リポジトリはIssue #1実装時にこれを見落とし、本番デプロイ・本番ユーザーテスト完走後に事後対応（Issue #13）した。実装直後・本番デプロイ後のいずれか早い時点で必ず確認すること。
...
## デモ共通UI必須4要素+GA4タグ（Issue #13・PR #14・2026-09-15対応）

本番デプロイ・本番ユーザーテストが完了した後、`20_開発/.claude/agents/demo-common-ui.md`が定める全デモ共通の必須要素が一つも実装されていないことが判明し、事後で追加した（`requirements.md`にこの節への言及が無く、実装工程に含まれていなかったため）。

- ページ最上部（`<header>`より前）のアンバーバナー（`src/frontend/components/layout.ts`の`demoBanner`、`.demo-banner`）
- ナビ右端の「← デモ一覧へ」戻るリンク（`https://rictaworks.jp/#demos`、`.app-nav-backlink`）
- 画面右下固定の「ご相談はこちら」ボタン（`https://rictaworks.jp/`、`.consult-button`）
- `/legal`ページ（`src/frontend/pages/legalPage.ts`。利用規約・免責事項・連絡先を1ページに集約）とフッターからのリンク
- GA4タグ（`G-C04W1XKS16`、`src/frontend/index.html`）

文言は全て`STRINGS`の`demo`・`legal`セクション（`src/frontend/strings/ja.ts`）に集約。回帰テストは`test/e2e/demoCommonUi.spec.ts`。タグ`v01.01.02`で本番反映済み・実機確認済み。
